...
<body>
<form role="form" action="<%=request.getContextPath()%>/web/login.action" enctype="application/x-www-form-urlencoded">
<div></div>
<div class="form-group" style="margin-top:50vh;" align="center">
	<table>
		<tr><td>用户名：</td><td><input name="user.username" placeholder="请输入用户名" class="form-control input"/></td></tr>
		<tr><td>密&emsp;码：</td><td><input type="password" name="user.password" placeholder="请输入密码" class="form-control input"/></td></tr>
...
<script>
	...
	*/
	function login(){
		var form = $('form');
		$.post('<%=request.getContextPath()%>/web/login.action', $(form).serialize(), function(data){
            debugger;
//			var res = eval('(' + data + ')');
			var res = data;
			var code = +res.code;
			...
			if(code && msg) {
				...
				location.href='<%=request.getContextPath()%>/system/index.action';
			}
		});
	}
	...
	*/
	function retister() {
		var form = $('form');
		$.post('<%=request.getContextPath()%>/web/doRegister.action', $(form).serialize(), function(data){
			$.showResultMsg(data);
		});
        <%--window.location.href = "<%=request.getContextPath()%>/web/register.action";--%>
	}
